...
for(cutoff in cutoff_list){
  ...
  for(pop in pop_list){
    res_all[[pop]] = get_percentile_pop(pop, array_list, trait_list= c("HEIGHT", "BMI", "Type_2_diabetes"), cutoff = cutoff)
  } 
  ...
  trait = "BMI"
    g = ggplot(data=df[df$trait == trait,] , aes(x = array_rename, y = pct_dif, fill = array_rename)) + geom_boxplot(outlier.shape = NA) + theme_light() + ylab("Percentile difference") + xlab("SNP array")  + facet_wrap(~ pop, nrow = 2) + theme(legend.position="non", axis.title.x = element_blank()) + guides(x = guide_axis(angle = 90))  +  scale_y_continuous(breaks=seq(0,100,5), limits = c(0,30) )
    
    # out_fn = paste0("../../PCT_ABS_dif_", trait , "_", cutoff, ".pdf") 
    out_fn = paste0("../../PCT_ABS_dif_", trait , "_", cutoff, ".jpg")  

    jpeg(filename = out_fn, width = 12, height = 9, units = "in", bg = "white",  res = 900,)
    #pdf(file= out_fn,  width=12, height=9)
    print(g)
    dev.off() 
  ...
  out_tab = paste0("../../PCT_ABS_dif_", trait , "_", cutoff, ".tsv")
...
# g = ggplot(data=df, aes(x = array_rename, y = pct_dif, fill = trait)) + geom_boxplot(outlier.shape = NA) + theme_light() + ylab("Percentile difference") + xlab("SNP array")  + facet_wrap(~ pop, nrow = 6) + theme(legend.position="bottom", axis.title.x = element_blank()) + guides(x = guide_axis(angle = 90)) +  scale_y_continuous(breaks=seq(0,1,0.1), limits = c(0,0.3) )
# pdf(file= "../../PCT_ABS_dif_all_merged.pdf",  width=12, height=20)
# g
# dev.off()


# trait = "BMI"
# g_BMI = ggplot(data=df[df$trait == trait,] , aes(x = array_rename, y = pct_dif, fill = array_rename)) + geom_boxplot(outlier.shape = NA) + theme_light() + ylab("Percentile difference") + xlab("SNP array")  + facet_wrap(~ pop, nrow = 2) + theme(legend.position="non", axis.title.x = element_blank()) + guides(x = guide_axis(angle = 90))  +  scale_y_continuous(breaks=seq(0,1,0.1), limits = c(0,0.3) )
...
# plot_pop_list <- function(pop_list, array_list){
#   p = list()
#   for(pop in pop_list){
#     df = get_percentile_pop(pop, array_list)
#     df = rename_array(df)
#     p[[pop]] = ggplot(data=df, aes(x = array_rename, y = pct_dif)) + geom_boxplot(outlier.shape = NA) + theme_light() + ylab("Percentile difference") + xlab("SNP array")  + facet_wrap(~ trait, nrow = 1) + theme(legend.position="bottom", axis.title.x = element_blank()) + guides(x = guide_axis(angle = 90)) + ggtitle(pop) +  scale_y_continuous(breaks=seq(0,1,0.1), limits = c(0,0.3) )
#   }
...
#     p[[2]] = p[[2]] + theme(legend.position="none") + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title.x = element_blank())
#     p[[3]] = p[[3]] + theme(legend.position="bottom", axis.title.x = element_blank(), axis.title.y = element_blank())
#     library(patchwork)
#     m = wrap_plots(p, nrow = 3, plot_layout(guides = "auto"))
#     return(m)
# }

# pop_list_1 = c("AFR", "AMR", "EUR")
# m1 = plot_pop_list(pop_list_1, array_list)

# pop_list_2 = c("EAS", "SAS", "VNP")
# m2 = plot_pop_list(pop_list_2, array_list)


# pdf(file= "../../Figure_4.pdf",  width=12, height=15)
# m1
# dev.off()

# pdf(file= "../../Figure_5.pdf",  width=12, height=15)
# m2
# dev.off()
...
# #########
# pop_list = c(pop_list_1, pop_list_2)

# plot_6_pop <- function(pop_list, array_list){
#   p = list()
#   for(pop in pop_list){
#     df = get_percentile_pop(pop, array_list)
#     df = rename_array(df)
...
#     p[[6]] = p[[6]] + theme(legend.position="bottom", axis.title.x = element_blank(), axis.title.y = element_blank())
#     library(patchwork)
#     m = wrap_plots(p, nrow = 6, plot_layout = "auto")
#     return(m)
# }

# m = plot_6_pop(pop_list, array_list)

# pdf(file= "../../PCT_ABS_dif_all.pdf",  width=12, height=20)
# m
# dev.off()