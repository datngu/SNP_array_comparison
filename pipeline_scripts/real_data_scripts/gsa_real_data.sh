...
bcftools annotate -x INFO,^FORMAT/GT GSAv3_24.rename.normalized.vcf.gz | sed 's/chr//g' | bgzip > GSAv3_24.rename.clean.vcf.gz
bcftools index -t  GSAv3_24.rename.clean.vcf.gz

gsa_vcf=GSAv3_24.rename.clean.vcf.gz
...
### copy reference set from pmra 
cp -r /dragennfs/area8/datnguyen/PRS_arrays_project/VN_1008_all/pmra_real_data/reference_set ./
...
mkdir real_24_sample

sample=shared_sample.txt
for i in {1..22}
do
    bcftools view $gsa_vcf -r $i -S ${sample} -o real_24_sample/chr${i}.vcf.gz -Oz &
done
...
for i in {1..22}
do
    bcftools index -t real_24_sample/chr${i}.vcf.gz &
done
...
mkdir simulated_24_sample

put /media/datn/data/1st_DC_PRS_array_project/array_annotation/annotation_all_hg38/global-screening-array-v.3/* /dragennfs/area8/datnguyen/PRS_arrays_project/VN_1008_all/gsa_real_data/simulated_24_sample
...
do
  ...
  position=simulated_24_sample/chr${i}.txt
  vcftools --gzvcf  $vcf_wgs --positions $position --recode --stdout | sed 's/0|0/0\/0/g' | sed 's/0|1/0\/1/g' | sed 's/1|0/1\/0/g' | sed 's/1|1/1\/1/g' | bgzip > simulated_24_sample/chr${i}.vcf.gz
  bcftools index -t simulated_24_sample/chr${i}.vcf.gz
done
...
process_imputed_file ()
{
  ...
  echo $'ID' $(bcftools view -h $fi | grep "^#CHROM" | cut -f10-) | sed -e 's/ /\t/g' > $out_dose
    query $fi -f '%CHROM:%POS:%REF:%ALT[\t%DS]\n' >> $out_dose
  gzip -f $out_dose
...
array_list="simulated_24_sample real_24_sample"
...
do
  for i in {22..1}
  do
    bash /dragennfs/area7/datnguyen/bioinformatics_tools/compute_cor/pipeline_compute_correlation_KHV.sh /dragennfs/area8/datnguyen/PRS_arrays_project/VN_1008_all/gsa_real_data/wgs_24_sample/ref_GT/chr${i}.txt.gz /dragennfs/area8/datnguyen/PRS_arrays_project/VN_1008_all/gsa_real_data/${array_dir}/chr${i}_imputed.dose.txt.gz /dragennfs/area8/datnguyen/PRS_arrays_project/VN_1008_all/gsa_real_data/${array_dir}/chr${i}_imputed.correlation.txt.gz &
  done
  wait
  echo "done $array_dir"
done
...
### copy results
get -r PRS_result /media/datn/data2gb/GitHub/SNP_array_comparison_full_data/gsa_real_data



# remote

get real_24_sample/*cor*gz  /media/datn/data2gb/GitHub/SNP_array_comparison_full_data/gsa_real_data/real_24_sample

get simulated_24_sample/*cor*gz  /media/datn/data2gb/GitHub/SNP_array_comparison_full_data/gsa_real_data/simulated_24_sample
